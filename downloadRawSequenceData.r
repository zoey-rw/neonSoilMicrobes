...
site = c("CPER","DSNY","HARV","OSBS","STER"); startdate="2015-06"; enddate="2019-06"; out.dir = "/projectnb/talbot-lab-data/zrwerbin/NEON_16S_ITS_data_construction/testing/raw_seqs/ITS"; check.size=F; tar_dir = "/projectnb/talbot-lab-data/zrwerbin/NEON_16S_ITS_data_construction/data/ITS/rawSeqs/recent/tar/";  amplicon="ITS"; fastq_dir = NULL; tar_dir = NULL

downloadRawSequenceData <- function(site = "HARV", startdate="YYYY-MM", enddate="YYYY-MM", check.size=F, amplicon="ITS", out.dir= NULL, fastq_dir = NULL, tar_dir = NULL) {
...
cat(paste0("\nDesired fastQ files are split into ", n.urls, " unique sequencing runs (tar.gz files) to download.\nIf these have previously been downloaded, set the `tar_dir` argument to the previous download location.\n"))
...
#  for (i in 10:10) {
  ...
  if (any(file_exists)) {
    ...
    #### DOWNLOAD tar files ####
    utils::download.file(url = as.character(u.urls[i]),
                         destfile = paste0(tar_dir, fileNms[i]))
  ...
  if(length(files.to.extract) == 0) next()
  ...
  cat(paste0("\nFiles from run: ", i, "/", n.urls, ": ", fileNms[i], " have been extracted.\n"))
