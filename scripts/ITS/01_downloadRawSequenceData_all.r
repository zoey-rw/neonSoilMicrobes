...
check.size=F
amplicon="ITS"
out.dir <- "/projectnb/talbot-lab-data/zrwerbin/NEON_16S_ITS_data_construction/raw_sequences_16S/recent/"
out.dir <- "/projectnb/talbot-lab-data/zrwerbin/NEON_16S_ITS_data_construction/raw_sequences_ITS/recent/"
...
saveRDS(dat, "/projectnb/talbot-lab-data/zrwerbin/NEON_16S_ITS_data_construction/allNEONmetadata.rds")
dat <- readRDS("/projectnb/talbot-lab-data/zrwerbin/NEON_16S_ITS_data_construction/allNEONmetadata.rds")
...
sequencing.dat$files_we_want <- sequencing.dat$processedSeqFileName
...
for(i in 1:length(u.urls)) { # loop through each URL
 # for(i in 42:length(u.urls)) { # for testing
  ...
  if(any(file_exists)) { cat(paste0("File: ", fileNms[i], " already exists. Skipping download.\n"))
    ...
    extractedLabIDs <- sapply(strsplit(files_extracted, split=paste0("_",amplicon)), '[', 1)
